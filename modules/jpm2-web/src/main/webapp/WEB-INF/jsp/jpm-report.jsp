...
<html lang="${locale.language}">
    ...
    <jpm:jpm-body showMenu="false">
        ...
        <c:if test="${not empty userSave}"><h4><a class="btn btn-danger confirm-true" id="btnDeleteCurrent" href="${cp}jpm/report/${reportId}/${savedReportId}/delete" title="Delete"><span class="fa fa-trash-alt"></span></a> ${userSave.name}</h4></c:if>
            <div class="row">
                <div class="col-lg-3 offcanvas">
                    <div>
                        <div class="input-group">
                            <span class="input-group-addon add-on"><spring:message code="jpm.reports.filter.from" text="First" /></span>
                    ...
                    <div class="input-group">
                        <span class="input-group-addon add-on"><spring:message code="jpm.reports.filter.count" text="Count" /></span>
                ...
                <div class="btn-group">
                    ...
                    <select id="sortField" style="padding:10px;">
                        <c:forEach items="${report.sortableFieldList}" var="fs" varStatus="st">
                            <option value="${fs}" ${(not empty savedReport and savedReport.sortField==fs)?'selected':''}><jpm:field-title entity="${report.entity}" fieldId="${fs}"  showHelp="false" /></option>
                        </c:forEach>
                    </select>
                </div>
                <hr class="hr-sm"/>
                    ...
                    <ul class="dropdown-menu" role="menu">
                        <c:forEach items="${fieldSearchs}" var="fs" varStatus="st">
                            <li role="presentation">
                                <a id="search-link-${st.index+1}" role="menuitem" tabindex="-1" data-field='${fs.key.id}' href="javascript:addSearch('${fs.key.id}');">${st.index+1}. <jpm:field-title entity="${report.entity}" fieldId="${fs.key.id}" showHelp="false"/></a>
                            </li>
                        </c:forEach>
                    </ul>
                </div>
                <div id="filterZone">
                    ...
                    <ul class="dropdown-menu" role="menu">
                        <li role="presentation">
                            <a role="menuitem" tabindex="-1" data-field='${fs}' href="javascript:addGroup('', '<spring:message code="jpm.reports.group.all" text="All" />');"><spring:message code="jpm.reports.group.all" text="All" /></a>
                        </li>
                        <li class="divider"></li>
                            <c:forEach items="${report.groupableFieldList}" var="fs" varStatus="st">
                            <li role="presentation">
                                <a role="menuitem" tabindex="-1" data-field='${fs}' href="javascript:addGroup('${fs}', '<jpm:field-title entity="${report.entity}" fieldId="${fs}"  showHelp="false" />');"><jpm:field-title entity="${report.entity}" fieldId="${fs}"  showHelp="false"/></a>
                            </li>
                        </c:forEach>
                    </ul>
                </div>
                    ...
                    <ul>
                        <c:if test="${not empty savedReport}">
                            <c:forEach var="f" items="${savedReport.groups}">
                                <li class='group' data-field='${f}'><jpm:field-title entity="${report.entity}" fieldId="${f}"  showHelp="false" /> <span class='closer fa fa-times'></span></li>
                                </c:forEach>
                            </c:if>
                    </ul>
                </div>
                    ...
                    <div class="btn-group" id='select-formula'>
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <c:forEach items="${report.numericFieldList}" var="fs" varStatus="st">
                                <li role="presentation">
                                    <a role="menuitem" tabindex="-1" data-field='${fs}' href="javascript:addFormula('${fs}', '<jpm:field-title entity="${report.entity}" fieldId="${fs}"  showHelp="false"/>', 'formulaZone', 'general-formula');"><jpm:field-title entity="${report.entity}" fieldId="${fs}"  showHelp="false"/></a>
                                </li>
                            </c:forEach>
                        </ul>
                    </div>
                ...
                <c:if test="${not empty report.descriptiveFieldList}">
                    <div id='select-descriptive-fields'>
                        ...
                        <c:forEach items="${report.descriptiveFieldList}" var="fs" varStatus="st">
                            <label><input type="checkbox" class="visibleField" value="${fs}" ${(not empty savedReport)?((savedReport.visibleFields.contains(fs))?'checked':''):'checked'}/> <jpm:field-title entity="${report.entity}" fieldId="${fs}"  showHelp="false"/></label><br/>
                            </c:forEach>
                    </div>
            ...
            <ul class="dropdown-menu" role="menu">
                <c:forEach items="${report.groupableFieldList}" var="fs" varStatus="st">
                    <li role="presentation">
                        <a role="menuitem" tabindex="-1" data-field='${fs}' href="javascript:addGraphic('${fs}', '<jpm:field-title entity="${report.entity}" fieldId="${fs}" showHelp="false" />', numericFields);"><jpm:field-title entity="${report.entity}" fieldId="${fs}" showHelp="false" /></a>
                    </li>
                </c:forEach>
            </ul>
        </div>
        <div id="graphicZone"><ul></ul></div>-->
            ...
            <div class="col-lg-9 mainzone">
                <div class="btn-group">
                    <button class="navbar-toggler btn btn-default" type="button"><span class="fa fa-bars"></span></button>
                    <button class="btn btn-default" id="btnHtml">HTML</button>
                    <button class="btn btn-success" id="btnExcel">EXCEL</button>

                    <!--<button class="btn btn-danger"  id="btnPdf">PDF</button>-->
                </div>
                <div class="btn-group">
                    <button id="search-dropdown" type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
